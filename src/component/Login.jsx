...
function Login() {
    ...
    const sendOtp = () => {
        ...
        webClient.post(API.SEND_OTP_ENDPOINT, { email: inputs.email },{timeout:10000})
            .then(response => {
    ...
    const handleErrorResponse = (error, context) => {
        if (error.response) {
            if (error.response.status === 400) {
                setError(`Invalid credentials. ${context} failed.`);
            } else {
                setError(`An unexpected error occurred during ${context}. Please try again later.`);
            }
        } else if (error.code === 'ECONNABORTED') {
            setError(`Request timed out during ${context}. Please try again later.`);
        } else {
            setError(`An unexpected error occurred during ${context}. Please try again later.`);
        }
    };
